...
{
    uint32_t sessionId;
    uint32_t paylaodSize;
    bool isAppMessage;
    bool isSecuredMessage;
} XcenaSpdmDescriptor;
 ...
 **/
uint8_t spdm_xcena_get_sequence_number(uint64_t sequence_number,
 ...
 **/
uint32_t spdm_xcena_get_max_random_number_count(void)
{
    return 0;
}
 ...
 */
spdm_version_number_t spdm_xcena_get_secured_spdm_version(
...
{
    *transport_message = (uint8_t *)message - sizeof(XcenaSpdmDescriptor);

    XcenaSpdmDescriptor* descriptor = *transport_message;
    descriptor->isAppMessage = false;
    descriptor->paylaodSize = message_size;
    if (NULL == session_id)
    {
        descriptor->isSecuredMessage = false;
        descriptor->sessionId = 0;
    }
    else
    {
        descriptor->isSecuredMessage = true;
        descriptor->sessionId = *session_id;
    }

    *transport_message_size = sizeof(XcenaSpdmDescriptor) + message_size;
...
{
    XcenaSpdmDescriptor* descriptor = (XcenaSpdmDescriptor*)transport_message;
    *message = (uint8_t *)transport_message + sizeof(XcenaSpdmDescriptor);
    *message_size = descriptor->paylaodSize;

    if (true == descriptor->isSecuredMessage)
    {
 ...
 **/
uint32_t spdm_transport_xcena_get_header_size(void *spdm_context)
{
    return sizeof(XcenaSpdmDescriptor);
}
